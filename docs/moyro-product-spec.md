...
moyro는 Mattermost의 공개 REST API v4, WebSocket 이벤트, 봇·웹훅·슬래시 명령과 익숙한 채널 중심 작업 흐름을 계승하면서, 내부는 Go·React·PostgreSQL 기반의 작은 모듈형 모놀리스로 유지하는 자체 운영형 협업 플랫폼이다.
...
- UI는 활성 정책이 하나도 없을 때 승인 메뉴와 상태를 표시하지 않는다.
...
/settings/ai
/settings/approvals/mine
/settings/approvals/review
/admin/overview
/admin/site
/admin/auth/keycloak
/admin/ai/providers
