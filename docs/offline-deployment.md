...
This guide describes how to install the v0.2.5 moyro service image in a network
...
For the `v0.2.5` release tag:

- Docker image: `moyro:v0.2.5`
- Downloaded file: `moyro-v0.2.5.tar.gz`
...
```bash
sha256sum moyro-v0.2.5.tar.gz
```

Load it without contacting a registry:

```bash
docker load --input moyro-v0.2.5.tar.gz
docker image inspect moyro:v0.2.5
```
...
v0.2.5 ignores forwarding headers by default. Add only the reverse proxy's
...
Mattermost AppError in v0.2.5. SMTP, S3, Redis, and link-preview toggles
...
Run exactly one moyro application container in v0.2.5. PostgreSQL may be
...
digests of short-lived SSO codes and browser bindings. v0.2.5 adds
`000012_sso_handoff_idempotency`, recording the completed session and exchange
time needed for the bounded response-loss recovery window. Startup applies both
through the checksummed migration ledger and transactions. Keep the pre-upgrade
database backup because an image-only rollback is not a supported way to remove
an applied migration.

Do not mix an earlier pre-moyro development build with a v0.2.5 node. Upgrade
...
```bash
bash scripts/verify-release.sh moyro:v0.2.5 moyro-v0.2.5.tar.gz
```
...
- SMTP delivery, S3 storage, Redis fan-out, and outbound link previews remain
  explicitly unsupported in v0.2.5 rather than reporting a false successful
