...
This guide describes how to install the v0.2.23 moyro service image in a network
...
For the `v0.2.23` release tag:

- Docker image: `moyro:v0.2.23`
- Downloaded file: `moyro-v0.2.23.tar.gz`
...
```bash
sha256sum moyro-v0.2.23.tar.gz
```

Load it without contacting a registry:

```bash
docker load --input moyro-v0.2.23.tar.gz
docker image inspect moyro:v0.2.23
```
...
v0.2.23 ignores forwarding headers by default. Add only the reverse proxy's
...
Mattermost AppError in v0.2.23. SMTP, S3, Redis, and link-preview toggles
...
Run exactly one moyro application container in v0.2.23. PostgreSQL may be
...
time needed for the bounded response-loss recovery window. v0.2.23 adds
...
Do not mix an earlier pre-moyro development build with a v0.2.23 node. Upgrade
...
```bash
bash scripts/verify-release.sh moyro:v0.2.23 moyro-v0.2.23.tar.gz
```
...
- SMTP delivery, S3 storage, Redis fan-out, and outbound link previews remain
  explicitly unsupported in v0.2.23 rather than reporting a false successful
