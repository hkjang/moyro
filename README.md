...
- Search, saved posts, public channel discovery, and a link-preview foundation
  (outbound previews are disabled by the offline-safe v0.2.23 runtime)
...
- A channel timeline with day separators, a "새 메시지" marker at the reader's
  ...
  up; the view opens on the newest message; scrolling to the top pages older
  history in without moving the reader, and the channel says when its start
  is reached
- Keyboard: `?` lists every shortcut the workspace implements, Alt+↑/↓ walk
  the sidebar, and ↑ on an empty composer edits the author's latest message
- Root posts carry an inline thread summary ("답글 N개 · 마지막 답글") served by
  the list endpoints as `reply_count`/`last_reply_at`; the context panel adds
  a member roster with presence and a pinned-message list; the composer
...
- KakaoTalk-style emoticons: 102 built-in stickers across nine packs — four
...
- Long conversations stay responsive: off-screen message rows are skipped by
  the browser rather than removed from the document, so jumping to a post by
  id, in-page find, and scroll anchoring keep working while layout cost stops
  scaling with the list
...
- An empty conversation introduces itself — room purpose, size, and creation
  day with links to the roster and info, or the other person and their
  presence for a direct message; the header's member count opens the roster;
  a user's own presence follows the socket, so the account menu reads
  "온라인" as soon as the connection is up
- While typing, the composer offers emoticons whose keywords match the words
  at the caret ("고마워" → 고마워요, "퇴근" → 퇴근!, "ㅋㅋ" → ㅋㅋㅋ); one tap sends
  the emoticon, and a text that was only the keyword is cleared with it
- A Content-Security-Policy on the web UI that allows scripts only from the
  ...
  slow-query log that names the statement
...
The supported v0.2.23 topology is one moyro application container connected to
...
After a successful provider callback, v0.2.23 sends the browser a five-minute,
